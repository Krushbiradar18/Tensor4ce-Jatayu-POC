...
def generate_explanation(state: AgentState) -> dict:
    ...
    llm_result = get_llm_explanation_details(prompt)
    recommendation = generate_recommendation(ml["risk_category"], ml["risk_score"])

    return {
        "llm_explanation": llm_result["text"],
        "llm_status": llm_result["llm_status"],
        "llm_provider_error": llm_result["llm_provider_error"],
...
def compile_result(state: AgentState) -> dict:
    ...
    return {
        "final_result": {
            ...
            "llm_status": state.get("llm_status", "not_called"),
            "llm_provider_error": state.get("llm_provider_error", ""),
