...
with open(_resolve_data_file("static", "ip_mock_map.json")) as f:
    IP_PATTERNS = json.load(f)["patterns"]
with open(_resolve_data_file("static", "fraud_blacklist.json")) as f:
    BLACKLIST = {e["pan"] for e in json.load(f)["blacklisted_pans"]}
with open(_resolve_data_file("config", "bureau_mock_rules.json")) as f:
    BUREAU = json.load(f)
