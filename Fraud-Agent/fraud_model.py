...
with open(ROOT / "static/ip_mock_map.json") as f:
    IP_PATTERNS = json.load(f)["patterns"]
with open(ROOT / "static/fraud_blacklist.json") as f:
    BLACKLIST = {e["pan"] for e in json.load(f)["blacklisted_pans"]}
with open(ROOT / "config/bureau_mock_rules.json") as f:
    BUREAU = json.load(f)
...
def train_model() -> Pipeline:
    ...
    with open(MODEL_PATH, "wb") as f:
        pickle.dump(pipe, f)
    print(f"[fraud_model] Trained on {X.shape[0]} samples, saved → {MODEL_PATH}")
    return pipe
