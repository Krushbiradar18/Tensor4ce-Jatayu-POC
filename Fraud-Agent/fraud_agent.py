...
def apply_rules(state: AgentState) -> AgentState:
    ...
    # Fast form fill (15-60s) — could be a returning applicant, but suspicious
    if 15 <= f["form_fill_time_seconds"] < 60:
        soft.append(f"fast_form_fill: {int(f['form_fill_time_seconds'])}s (normal is 90-600s)")
...
def build_output(state: AgentState) -> AgentState:
    ...
    if level == FraudLevel.HIGH_RISK:
        ...
        expl = (
            f"ESCALATE — {len(soft)} risk signal(s) detected: {'; '.join(soft[:4])}. "
            f"Model fraud score: {prob:.2f}. "
            "Needs manual review by a loan officer before approval."
        )
    else:
        if soft:
            expl = (
                f"PROCEED — Application appears clean. "
                f"{len(soft)} minor signal(s) noted ({soft[0]}) — "
                "within normal tolerance, no action required."
            )
        else:
            expl = "PROCEED — No fraud signals detected. Application can continue to credit assessment."
